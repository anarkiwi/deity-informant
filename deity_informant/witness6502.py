...
write stream is witnessed with no evaluator in the trust chain; the rest refuses.
"""
...
CTRL_BYTES = 3  # fault flag, saved stack pointer, compare accumulator
...
_STMT_REFUSALS = {
    "opsw": "the observed-variant SMC dispatch switch (state-variable opcode switch)",
    "swg": "the observed-variant SMC dispatch switch (computed-goto arm table)",
    "swc": "the observed-variant SMC dispatch switch (computed-call arm table)",
    "dgoto": "the computed transfer (dgoto: a jump through a computed target)",
    "igoto": "the computed transfer (igoto: a jump through an image vector)",
    "dcall": "the computed transfer (dcall: a call through a computed target)",
    "dbr": "the computed transfer (dbr: a guarded jump through a computed target)",
...
class _Gen:
    ...
    def __init__(self, prog, org):
        ...
        self.pcs = {s[1] for _e, _pa, _r, st in prog.procs for s in _walk(st) if s[0] == "label"}
        self.lw = self._local_widths()
        self.fault, self.saved, self.cmp = org, org + 1, org + 2
        base = org + CTRL_BYTES
        self.locs = {}
