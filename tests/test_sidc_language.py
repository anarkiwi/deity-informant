"""SIDC language (docs/sidc-language.md): property-based round-trip laws over
generated models, version policy, and dumps/loads vocabulary."""
...
# ---- expression algebra strategy (the surface grammar of fmt_expr/parse_expr) --
...
# simplify -> the algebra's canonical form (the domain fmt_expr/parse_expr invert).
_expr = st.recursive(_atom, _extend, max_leaves=6).map(E.simplify)
_ADDR = st.integers(0x0800, 0xBFFF)


# ---- block / terminator / model strategies (canonical form) -------------------
def _canon_events(raw):
    """Merge adjacent @cyc and drop zero costs, exactly as emit renders them."""
    out = []
    for ev in raw:
...
# A terminator carries at most one dynamic target (expr; static addr then None).
_TERM = st.one_of(
    ...
    st.tuples(_ADDR, _ADDR).map(lambda t: ("jsr", t[0], t[1], None)),
...
def _model(draw):
    ...
    for pc in plain:
        ...
        mem0[pc] = op0  # plain label is "$pc": parse recovers op0 from the image
    ...
    for addr, val in draw(st.lists(st.tuples(_ADDR, st.integers(1, 255)), max_size=8)):
        if addr not in plain:  # preserve plain op0 == mem0[pc]
            mem0[addr] = val
    return stext.TextModel(
        mem0,
...
def _key(m):
    """Comparable structure a serialisation round trip must preserve."""
    return (
        m.init,
        m.play,
        getattr(m, "subtune", 0),
...
def test_roundtrip_identity_and_fixpoint(m):
    ...
    text = stext.dumps(m)
    back = stext.loads(text)
    assert _key(back) == _key(m)
    assert stext.dumps(back) == text  # dumps(loads(dumps(m))) == dumps(m)


@settings(max_examples=100, deadline=None)
@given(_expr)
def test_expr_roundtrip(e):
    assert stext.parse_expr(stext.fmt_expr(e)) == e


def test_dumps_loads_are_emit_parse():
    assert stext.dumps is stext.emit and stext.loads is stext.parse


def test_header_carries_current_version():
    m = stext.TextModel(bytearray(0x10000), 0x1000, 0x1003, {}, {})
    assert stext.dumps(m).startswith("sidc %d\n" % stext.SIDC_VERSION)


def test_future_major_fails_cleanly():
    m = stext.TextModel(bytearray(0x10000), 0x1000, 0x1003, {}, {})
    future = stext.dumps(m).replace("sidc %d" % stext.SIDC_VERSION, "sidc 999", 1)
    with pytest.raises(stext.SidcVersionError):
        stext.loads(future)


@pytest.mark.parametrize("bad", ["", "not a sidc doc\n", "sidc\n", "sidc x\ninit $1000\n"])
def test_non_sidc_document_rejected(bad):
