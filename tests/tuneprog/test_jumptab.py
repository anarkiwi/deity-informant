...
def _guarded(c, again=()):
    """One block that branches on ``c`` into two, as :func:`jumptab._proved_range` sees it.
...
def test_a_sign_test_bounds_the_index_on_both_of_its_arms():
    ...
    assert jumptab._proved_range(blocks, "t", Var("x"), preds) == (0, 128)
    assert jumptab._proved_range(blocks, "f", Var("x"), preds) == (128, 256)
    assert jumptab._proved_range(blocks, "f", Var("y"), preds) is None  # another value is free


def test_a_compare_and_an_equality_bound_the_index():
    blocks, preds = _guarded(Bin("<", Var("x"), Const(21), 1))
    assert jumptab._proved_range(blocks, "t", Var("x"), preds) == (0, 21)
    assert jumptab._proved_range(blocks, "f", Var("x"), preds) == (21, 256)
    blocks, preds = _guarded(Bin("==", Var("x"), Const(7), 1))
    assert jumptab._proved_range(blocks, "t", Var("x"), preds) == (7, 8)
    assert jumptab._proved_range(blocks, "f", Var("x"), preds) is None  # != is not an interval
...
def test_a_branch_above_the_name_s_last_assignment_proves_nothing():
    ...
    assert jumptab._proved_range(blocks, "t", Var("x"), preds) is None
...
def test_a_join_proves_nothing_the_index_must_hold():
    ...
    assert jumptab._proved_range(blocks, "join", Var("x"), preds) is None
...
def test_a_folded_store_writes_its_cell_once_per_copy():
    proc, rgn = _merged()
    writers = jumptab._writers(proc, jumptab._let_defs(proc), rgn)
    assert sorted(writers) == list(CELLS)
    got = jumptab._source(0x2010, writers, rgn, bytes(0x10000), jumptab._let_defs(proc))
...
def test_a_copy_reads_the_column_entry_its_own_index_names():
    proc, rgn = _merged()
    defs = jumptab._let_defs(proc)
    e = defs["t"]
    assert [jumptab._as_copy(e, j, defs, rgn).a.a.v for j in range(4)] == list(BASES)
    assert jumptab._as_copy(e, 0, defs, rgn).a.b == Var("i")  # the index keeps its name
...
def test_a_proven_index_may_cut_into_a_copy_s_table_but_never_slide_it():
    ...
    src = [_tab((0, 21))]
    assert list(jumptab._table_domain(src, ext, 64)) == list(range(0, 21))
    assert list(jumptab._table_domain(src, ext, 64, (4, 256))) == list(range(4, 21))
    assert list(jumptab._table_domain(src, ext, 64, (0, 8))) == list(range(0, 8))
...
def test_a_table_the_extent_walked_back_past_starts_where_its_siblings_do():
    ...
    src = [_tab((129, 21))]
    assert list(jumptab._table_domain(src, ext, 64)) == list(range(129, 150))
    assert list(jumptab._table_domain(src, ext, 64, (128, 256))) == list(range(129, 150))
