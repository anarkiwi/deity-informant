...
- **tracker layer** — the final step, consuming a frame program and nothing else. A tune becomes a graph of one primitive, the triggered generator `(transfer, trigger, route)` with `DIV`/`LOOKUP`/`SELECT`/`RAMP`/`EDGE`/`RAW`; note lanes are equal-tempered semitone indices inverted from the **declared** pitch table, ctrl/ADSR is a **declared** instrument-bank lane read at a recovered row (for ctrl, with the gate bit riding that row's waveform byte), and every write a generator does not yet produce stays in an explicit `RAW` residual. Verified by one law — the graph's canonical projection must equal frameprog's evaluator, frame-for-frame — so coverage is reported, never assumed. Spec, primitive and measured coverage: [docs/tracker.md](docs/tracker.md).
