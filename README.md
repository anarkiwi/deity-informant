...
- `docs/prototype-lifter.md` — the lift from certified artefacts to a trackerprog: the schedule derived (B6), the tick lowered and T1's accumulators joined (B7) on two families, both rendered at 0 divergences over their whole horizons with 0 hints, the coverage, the field-by-field diff against the hand objects, and the limits the pass does not change.
