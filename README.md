...
- `docs/prototype-lifter.md` — the lift from certified artefacts to a trackerprog: the schedule derived (B6), the tick lowered and T1's accumulators joined (B7) on three families, two rendered at 0 divergences over their whole horizons and the third to tick 113 of 8,236, all with 0 hints; a tick of several procedures inlined, its register file and its prologue derived; the coverage, the field-by-field diff against the hand objects, and the limits the pass does not change.
