...
- `docs/trackerprog-backlog.md` — the open trackerprog work (mechanism, size, acceptance), the rules the nine transliterations settled, and the presentation gaps still to close.
- `docs/trackerprog-review.md` — a critical review of the universal player and its spec against the nine transliterations: the one-family mechanisms in the player, the player idioms the objects still carry, the dead and duplicated procedure, the render profile, the spec rows the code contradicts, and ten ordered items with poison-harness acceptance.
...
- `docs/prototype-blackbird-trackerprog.md` — lft's Blackbird (*Quintessence*) by hand as a trackerprog: the seventh family on the same universal player and **the first that cost the player no line at all**, certified at 0 divergences over the whole 10,426-tick horizon (208 s); the score is one LZ stream in three ring buffers and §6's materialisation drops all of it; the front-end fix the tune needed first (a site's class is its envelope's, an address's landing is the address's) and the voice order §2 drops that the harness was not dropping.
...
- `docs/prototype-walker-trackerprog.md` — Martin Walker's *Chameleon* by hand as a trackerprog: the eighth family on the same universal player, certified at 0 divergences over its whole 8,052-call horizon (80.3 s, a complete front-end certificate the tune did not have before); four modulators unrolled by modulator and not by voice, two of which sum into one frequency offset, which is why `amplitude` gained a counted turn; a score that is the C64 keyboard it was typed on, and an engine block `init` never clears, so the image's residue is the initial state.
- `docs/prototype-galway-trackerprog.md` — Martin Galway's *Comic Bakery* by hand as a trackerprog: the ninth family and the last of the anatomy's nine, certified at 0 divergences over **all fourteen subtunes** (29,911 ticks, write-for-write identical per register) on a front-end certificate the tune did not have; a counted loop that **nests**, because the score pushes it on the same 8-deep stack its calls use, and a `stop` that ends a voice's *sequencer* and leaves its engine playing the note out — which is what the eight sound-effect subtunes are made of.
...
- `tools/trackerprog_blackbird.py` — the same for lft's Blackbird: the tables come off the post-init image, but the score does not exist until the player has expanded its LZ stream, so the tool reads each row's decoded tokens off the tune's own per-voice cells at the tick its tokenizer finishes them (`--source docs/certificates/lft-quintessence.json --certify`).
...
- `tools/trackerprog_walker.py` — the same for Martin Walker's *Chameleon*: it reads every datum off the post-init image, including the 119-byte engine block `init` never clears, and materialises the typed-keyboard tracks into one semitone or drum per row (`--source docs/certificates/walker-chameleon.json --certify`).
- `tools/trackerprog_galway.py` — the same for Martin Galway's *Comic Bakery*: it simulates the three byte-code sequencers over the horizon to intern the instrument record each note copies (a `Moke` builds the record, a `DMoke` pokes the live engine, and only the second survives as a command), and states the two piecewise-linear generators as one accumulator per segment (`--song N --source docs/certificates/galway-comic-bakery.json --certify`).
- `tools/trackerprog_sizes.py` — section 9.1's measurement: every certified subtune's object against `xz -9e` of its tune's own PSID load band, over the same thirty builds, with each row stating how many of the tune's subtunes it covers.
- `tools/trackerprog_poison.py` — the poison harness section 7's method names: an object, a stated mutation (`--drop PATH`, `--set 'PATH=JSON'`, `--poison <name>`) and a build set, rendered both ways over each build's whole horizon, reporting differing ticks, the sites the mutation matched and the first divergence; the registry is the nine families' thirty certified builds and 332,358 ticks, every horizon read from `docs/certificates/`.
