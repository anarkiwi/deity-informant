...
- **Player-known instrument columns**: `instr().get(...)` reads `prelude`,
  ...
  **Closed by R8**, with `adsr` decided by measurement and not by argument: a
  note-on that emits it reproduces SID Wizard alone (§7 R8), so §3.5's box is the
  six the player reads and `adsr` is the family column six families read as
  `{"ins": "adsr.0"}`. Every column no expression, no player line and no print
  line reads is gone, under a hermetic test that walks all thirty objects.
...
instrument bytes and 51 % of Galway's. **The copies went with R8** — a row list
several instruments carry is one declared stream they name, and a column all of
them carry is `meta.instrument` — and the if-conversion did not: the rows are
the same rows, stated once.
...
- JCH: AD/SR three ways (an edge write, an `@ad` cell, and `writeout` sending
  ...
  **`prelude.early` and the record's `flags vol filter pulse` struck by R8**.
- SID Wizard: `vibdelay arpsped chord wave gate_off` on the record, read by
  nothing because `on_note` bakes the same bytes — **struck by R8** with
  `pw_index` and `flt_index`, `wave` staying as the print's own column;
  `vibracnt` precomputed *and*
...
| §2 "order between voices dropped" | `meta.voice_order` changes the render through shared cells (§3.1); the certificate says the boundary is dropped and the object carries it. **Closed by R9**: `attest` and `certify` group a tick's edges per voice and compare each voice's own sequence, so what is dropped is the *interleave* — the `dropped` list says so now, in both modules and in §2, and §2 states that the order the voices run in is a datum and decides the render wherever two voices meet |
...
| §3.3 `Step.point`, `Step.op`, `Step.run` | `point` is read by `inline` only (B5); `op` and `run` by `stream_step` only; `on_note` rows cannot `op` and a stream row cannot `point` | **Closed by R9**: §3.3 states the two readers and gives each field its reader and the families that write it — `sets` both, `when` the row's under `runstream` and the stream's own under `stream_step`, `point` `runstream` alone, `hold`/`next`/`jump`/`op`/`run`/`trap` `stream_step` alone |
| §4's `tick()` pseudocode | omits the flush (512–514), the prologue (520–522), the channel before the voices (523), the stop (515–519), `channel_after` and `channel_commit`. A reader holding the code against §4 holds it against a sketch. **Closed by R9**: the block is `tick`/`voice`/`clock`/`channel`/`channel_commit`/`commit` and a test asserts it names those six and no others, each with a `def` in the player |
...
| §9.1 "the sound half doubles the total, 95 % of *Knob at Night*" | *Knob at Night*'s object is `wrapdata`: the rip wrapper's own 4-byte-per-frame table, 8,577 rows, read through `dptr`. That is tune data and not the sound vocabulary; the sentence attributes it to instruments, streams and accumulators. **Closed by R9, measured**: 598,626 of the `rest` column's 616,634 raw bytes and **12,636 of the object's 16,252 `xz`** are that one stream, and the column with it taken out is 2,936 |
...
| R8 | the instrument schema | either the player reads `adsr` (an instrument's note-on emits it, and the tools stop spelling it in `on_note`) or §3.5 says what is true: an instrument is a family record and six player names. The 62-copy `ENGINE` list and the 11-copy `on_note` are the same question — a record the tools inline per instrument wants a reference. **Landed #341**, and **`adsr` was decided by measurement, not by argument**: a player whose note-on emits it in `commit_order`'s place, with the tools' own `ad`/`sr` sets struck, reproduces **SID Wizard alone** — **0 of 8,084** and **0 of 14,465** with its 22 and 42 inline sets removed — while JCH's note-on puts `ad`, `sr` and `ctrl` in **one act** (**257 of 2,401** on *Guldkorn*, the build with no image; 0 of 8,577 on *Knob at Night*, whose image absorbs the second write) and Walker's puts them beside sixteen other cells (**346 of 8,052**); and Hubbard, Blackbird and Galway write theirs from a *stream* at another point in the tick, which the rule breaks outright (**1,543 of 11,780**, **1,428 of 10,426**, **1,327 of 9,450**). One family is not a player rule, so §3.5's box now names the six the player does read — `prelude on_note accs pitch transpose pw`, with `transpose` and `pw` marked with their single families — and `adsr` is what it always was, the family column all seven that carry it read as `{"ins": "adsr.0"}`; Galway spelled it *and* `ad`/`sr` and now spells it once. The reference form is **one shape for a row list wherever the grammar puts one** — a declared stream's name or the rows themselves, for a `prelude`, an `on_note` and a command's `rows` alike — so the prelude's `{"stream": …, "early": …}` record is gone with the `early` that restated `meta.tempo.early`, and `rowsource`/`runstream` are the one pair that resolves and runs all four places, `rows()` having gone with the second procedure. What every instrument of a family shares, the family states once: **`meta.instrument`**, the record each instrument extends and each overrides field for field. The 62-copy `ENGINE` list was six families' — Galway 62, SID Wizard 11 and 21, GoatTracker 2 30, JCH 19, Blackbird 15, Walker 11 — and none states it now; Walker's and Blackbird's `on_note` and Hubbard's `on_note` and `prelude` are declared streams. The unread columns go with them, **checked hermetically over all thirty cached objects** rather than asserted: SID Wizard's `vibdelay arpsped chord pw_index flt_index gate_off`, JCH's `flags vol filter pulse`, Blackbird's `note`, Walker's `filter`, Galway's `ad`/`sr` and two of its three spellings of one waveform byte, which are `{"or": [wave, 8]}` and `{"and": [wave, $F7]}` now. Measured: **0 differing of 332,358** over all thirty builds against the merge base's digests, every object rebuilt from all seven edited tools and every render recomputed. The instrument half falls 35,321 → 14,480 raw bytes on Galway, 25,334 → 12,876 on JCH, 25,262 → 11,355 on SID Wizard, 12,527 → 2,758 on Walker, and 1,012 → 840, 948 → 812, 1,164 → 1,032, 1,032 → 548 under `xz -9e`: **the copies were most of what the compressor was already removing**, so §9.1's whole-object column moves by 0 to 3 % and this row's value is the schema and not the size. 6,000 ticks in one process, best of three, *Comic Bakery song 1* **7,014 → 8,304** ticks/s — 62 instruments naming one arm list compile it once — *End of the World* 7,189 → **7,655**, *Chameleon* 8,564 → **8,957**, *Je suis Linus* 14,076 → **14,258**; `universal.py` 1,399 → **1,398**, `printer.py` 625 → 633, hermetic coverage 97 % and 100 % | medium |
| R9 | the spec | §4's pseudocode completed; §2's act rule and `voice_order` boundary stated; §9.1's *Knob at Night* row attributed to `wrapdata`; §2.1's table carried into B8 with its readers, so the B8 census is the code's and not the schema's. **Landed #344.** §4's block is `tick`/`voice`/`clock`/`channel`/`channel_commit`/`commit` written against the code, with the flush's own guards, the prologue's spent tick, the clock's `stepped` and the `{stream}` phase a spent tick still runs; §4.1's phase table says what makes `fetch` and `prelude` run and what `row` leaves. §3.3 states the **two readers** of a step and gives each field its reader and its families (`sets` both; `when` the row's under `runstream` and the stream's own under `stream_step`; `point` `runstream` alone; `hold`, `next`/`jump`, `op`, `run`, `trap` `stream_step` alone), measured over the thirty cached objects. §3.6 names the row program's six steps as a table, §5 the eight voice cells the player declares and the nine names `cell()` answers itself. §2's `dropped` list, in `attest.py` and `certify.py` alike, now says *the interleave between voices of one tick's writes*, and §2 says outright that the order the voices run in is a datum the render depends on. §9.1's row is `wrapdata`, measured. Six new checks in `tests/trackerprog/test_schema_doc.py`, each reading the player's own source: `test_the_cell_box_names_the_players_own_state_vector`, `test_the_cell_box_names_what_cell_answers_itself`, `test_the_row_program_box_names_every_step_row_step_runs`, `test_the_phase_table_names_every_phase_the_player_has_and_the_stream_phase`, `test_section_4s_pseudocode_names_every_procedure_one_tick_runs`, `test_the_step_table_gives_each_field_the_reader_the_player_gives_it` | small |
