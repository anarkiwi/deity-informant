...
| pulse run (unbounded) | target pw, `delta const(k) + carry(site)`, `bound` **`projected`** at 12 bits | Hubbard: an **8-bit** add on `pw_lo` with the carry **live from the vibrato block** — `ins.pw_lo += ins.pspeed + C  # C inherited from $51FA` (commando-floor:222-224, `+ carry` at commando.md:394); the 12 bits come from the store's `& $F` (commando.md:380). defMON: `voice[v].pw_lo -= (b101E + (1 - carry_2))` with `carry_2` produced by the freq add above it (automatas.md:427-447) — but that carry is **0 on all 17,235 sweep steps of both certified horizons**, so it distinguishes no observation and the row stays Hubbard's alone (defmon-trackerprog §8). These are the writes that make both Commando subtunes aperiodic (architecture §5.2), rendered exactly, aperiodicity included |
...
claim. Both landed; of the three citations the first two held and the third did
not — defMON's carry is 0 on every sweep step of both horizons, so §5's
`carry(site, flag)` row is still Hubbard's alone
