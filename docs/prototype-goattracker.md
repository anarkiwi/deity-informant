...
Hubbard's *Commando*: `MUSICIANS/L/Linus/Je_suis_Linus_le_salaud.sid` and
...
|---|---|---|
| the ghost block printed as `timer_4[v]`, the flush loop as `sid[v].freq_lo = timer_4[v]` | a region a loop copies byte-for-byte into `$D400..` is a **SID image**: `sidw($D400+i, load(R, base+i))` with one index expression gives the delta from region byte to register, and every access to `R` prints by the register it mirrors (`ghost[v].ctrl`, `ghost.mode_vol`) | `recover.image_copy`, `printer.regcell` |
| `T16F9[$16F8 + y]` — the operand printed instead of the index | a region records the address its index counts from (**origin** = operand + the smallest index observed); indices print from it, so a 1-based table reads `T[y]` and its look-ahead sibling `T[y + 1]` | `regions._origin`, `Rgn.zero`, `printer.addr_of` |
...
| G3 | front end | 437 sites, 73 regions (29 state, 43 const), 14 procedures, 230 blocks, 580 statements | 433 sites, 73 regions, 14 procedures, 225 blocks, 569 statements |
| G4 | ghost image | one `sid_image` region `$14CA`, 25 bytes, delta `$D400-$14CA`; the flush prints `for v in 24..0: sid[v] = ghost[v]` | same at `$B0F5` |
| G5 | patched low-byte dispatch | `switch` over `$1289`/`$1295` (**9** distinct tick-0 handlers of the table's 16), `$131E` (4 of 5 effects), `$1445` (1); every high byte constant, every default `trap` | 10 tick-0 handlers, 4 effects |
...
| G10 | structuring | **0** `goto`, **0** `sp`, 12 `trap 'untaken'` arms (branch directions the song never takes) in 1,179 printed lines / 26 procedures | 0 `goto`, 0 `sp`, 15 traps |
| G11 | cost | trace 12,000 calls in 23 s CPU, verify 8,236 calls in 0.8 s (10,576 calls/s), front end + presentation ~1 s: one invocation, under the 60 s budget | trace 24 s, verify 1.0 s |
| G12 | genericity | Automatas and Commando certify unchanged by the same code (`tests/tuneprog/test_hvsc*.py`, 18 tests) | — |
...
state     voice[3] stride 7, 12 fields
          ...
          cursor_1461 $1461 42 bytes cursor          # blocks A+B, one region
          b1289 b1295 b131E b1445 (patched jump cells) ; b1096 b10AC b110D
          cursor_1141 timer acc b118F b1194 b1310 b131A (the SMC immediates)
const     FREQ_LO $14E3 / FREQ_HI $1543 96 entries 12-TET ; T144A 16 bytes (the
          tick-0 jump table) ; T16F9 wavetbl 2-based ; T16BD.. nine instrument
          columns, 1-based, stride 30 ; T18B7.. the patterns
...
tick():                                  # $1003, 12,000 calls
    ...
    if b110D < 0:                        # init pending? ($FF = running)
        ...
        row_advance2()                   # the init path: zero A+B, 3x initchn
        return
...
row_apply(x):                            # $11A4 mt_execchn, 35,997 calls
    ...
    if voice[x/7].timer_2 == 0:                     # tick 0
        b1289 = T144A[cursor_1461[5 + x]]           # patch both JSR operands
        b1295 = b1289
        if cursor_1461[3 + x] == 0:                 # pattptr == 0: sequencer
            ptr = voice[voice[x/7].b148B].b15A3
            ptr[1] = voice[voice[x/7].b148B].b15A6
            t3 = T1875[((ptr[1] << 8) | ptr) + cursor_1461[x]]
        ...
        if cursor_1461[$17 + x] == 0:               # no new note
            p_1291(x=x, r5=t9)                      # -> the tick-0 command
            return
        else:
            voice[x/7].freq_lo_idx = (cursor_1461[$17 + x] - $60)
            ...                                     # load the instrument
...
p_1291(x, r5):                           # $1291, 2,688 calls
    switch b1295:                        # the patched JSR: 7 targets here
        case $1006:
            p_1013(x=x, y=r5)
            p_1297(x=x)
            return
        case $100C:
            p_100C(a=cursor_1461[6 + x], x=x)
            p_1297(x=x)
            return
        ...
p_1297(x):  # mt_waveexec   p_1327: mt_wavenote   p_1339: storefreqhi
p_1363/p_1373: the pulse program   p_1382: the row fetch + hard restart
p_1406/p_140F: pattptr, then ghost[v].ctrl = wave & gate
```
...
- **The jump tables are not enumerated statically.** The switches carry the
  targets the trace took (9 of 16 tick-0 commands, 4 of 5 effects); the remaining
  arms could be read out of the table region and marked unverified, the way
  `cfg._writer_variants` already does for opcode cells.
...
- **`p_XXXX` helper names.** A promoted tail is named after its address; the
  anatomy's names (`mt_loadregs`, `mt_done`) need the family dictionary.
- **The 42-byte block prints as `cursor_1461[$17 + x]`.** Readable, but the field
  names of blocks A and B (`newfx`, `newparam`, `waveptr`, `pulsetime`…) are only
  visible as offsets.
