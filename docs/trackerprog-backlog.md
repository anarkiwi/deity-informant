...
| --- | --- | --- | --- | --- |
| B6 | is the schedule recoverable? **Settled for three families** ([prototype-lifter.md](prototype-lifter.md) §2.1): the hypothesis holds, and §3.6's clock is derived whole — the counter a guard on the fetch's own path reads and a store steps, its `step`, a `boundary` that is a guard *list*, its `reset` clauses, and a divider told from the clock by comparing against the cell **its own reload reads**. Hubbard's every datum agrees with the hand's but one (the segment before the row is a `{stream}` phase, not `prelude`); JCH's `voice_order`, `commit_order`, `tempo.cell`, `step` and `reset` agree and four differ, three of them because the lift's `row` phase is the fetch region and it has no `stage`; GoatTracker 2's `voice_order`, `voices`, `shadow` (25 registers, in order), `state0.prologue`, `tempo.cell` and `step`, `rate` and `instruments` agree and five differ — `commit_order`, which a shadow makes unobservable, the boundary and `meta.tick` and `row_consumes_tick` for the row-phase reason, and the tuning's reach. Hubbard's and JCH's objects certify at 0 either way | medium | for each of the nine, the derived schedule against the hand-written one, agreeing or differing by a named datum. **3 of 9 done** |
| B7 | lower the tick, do not classify it. **The lowering lands on three families and the recognition on the plane T1 states** ([prototype-lifter.md](prototype-lifter.md) §2.2-§2.5, §4): Commando song 1 lifts to an object with no `program` key that `universal.py` renders at **0 divergences over 11,780 ticks and 133,109 writes**, from the certified artefacts and **0 hints** — 86 store sites into 86 rows and 5 `Acc` records, 4 refused, all three of T1's accumulators joined, 2.46× the load band against the hand's 1.36×. *Guldkorn Intro* lifts with **0 hints** and 1 named refusal — 132 store sites into 124 rows, 19 tables materialised as streams of their own bytes, 0 of T1's five joined — and renders **0 divergences over its whole 2,401-tick horizon and 63,229 writes**, 3.46× against 1.90×. *Je suis Linus* (GoatTracker 2) lifts with **0 hints** and 13 named refusals — a tick of several procedures inlined and its two runs of calls rerolled into the voice loop they are, §3.1's register file and its flush derived from T0, `state0.prologue` derived from the first call — and renders **113 of its 8,236 ticks**; the 114th diverges by one register pair, the slide's own borrow, whose input is one of the thirteen refusals. 4.71× against 2.18× | large | the lift emits an object `universal.py` renders, 0 divergences, no `program` key; coverage stated per family. **3 of 9 lifted, 2 of 3 certified whole; the residues are §8** |
