...
| B7 | lower the tick, do not classify it. **The lowering lands, the recognition does not** ([prototype-lifter.md](prototype-lifter.md) §2.2, §4): Commando song 1 lifts to an object with no `program` key that `universal.py` renders at **0 divergences over 11,780 ticks and 133,109 writes**, from the certified artefacts and **0 hints**. 79 of 86 store sites lower into `sets` rows of `all: True` streams, 3 into `Acc` records, 4 are refused; of T1's three accumulators one lands as an `Acc` and two stay as lowered `sets`. The object is 2.31× the load band against the hand's 1.36×, and that gap is the recognition pass | large | the lift emits an object `universal.py` renders, 0 divergences, no `program` key; coverage stated per family. **1 of 9 done; the recognition pass open** |
...
[prototype-lifter.md](prototype-lifter.md) §7.
