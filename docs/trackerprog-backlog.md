...
| --- | --- | --- | --- | --- |
| B6 | is the schedule recoverable? **Settled for one family** ([prototype-lifter.md](prototype-lifter.md) §2.1): the hypothesis holds. `trackerprog/schedule.py` derives Hubbard's `voice_order`, `commit_order`, `tempo`, `row_consumes_tick` and `meta.tick` from the certified tick alone, and every datum agrees with the hand's but one — the segment before the row is a `{stream}` phase, not `prelude`, because the lift keeps the tune's divider as a cell as well as `tempo.rate`. Taken *maximally*, the rule cuts the machine again at the drum's gate write, which the object does not need: `runstream` already makes each row its own act. What remains is the other eight families | medium | for each of the nine, the derived schedule against the hand-written one, agreeing or differing by a named datum. **1 of 9 done** |
| B7 | lower the tick, do not classify it. **The lowering lands and the recognition lands for the plane T1 states** ([prototype-lifter.md](prototype-lifter.md) §2.2, §2.3, §4): Commando song 1 lifts to an object with no `program` key that `universal.py` renders at **0 divergences over 11,780 ticks and 133,109 writes**, from the certified artefacts and **0 hints**. 86 store sites lower into 80 rows of `all: True` streams and 5 `Acc` records, 4 are refused; **all three of T1's accumulators** join into §5 records — `repeat` with a `flag`, `field` with a `phase bit`, and a `delta` carrying the carry the first one leaves. The object is 2.16× the load band against the hand's 1.36× (was 2.31×), and the residue is §7's: 80 rows of arithmetic no certified plane names as an accumulator | large | the lift emits an object `universal.py` renders, 0 divergences, no `program` key; coverage stated per family. **1 of 9 done; the four effects T1 does not name still lower as rows** |
...
run by `tools/tuneprog_trackerprog.py`; the four things a second family needs are
[prototype-lifter.md](prototype-lifter.md) §8, and what the recognition does not
change is its §7.
