...
| --- | --- | --- | --- | --- |
| B6 | is the schedule recoverable? **Settled for three families** ([prototype-lifter.md](prototype-lifter.md) §2): the hypothesis holds and §3.6's clock is derived whole. Hubbard's every datum agrees with the hand's but one (the instrument's lead-in is a `{stream}` phase, not `prelude`), and the clock cell is the player's own `rowsleft` where the fetch reloads it; JCH's `commit_order`, `tempo.cell`, `step` and `reset` agree. A tick of several procedures is no longer inlined, so GoatTracker 2 and SID Wizard refuse at B6 by name (`the tick reaches no fetch region`) | medium | for each of the nine, the derived schedule against the hand-written one, agreeing or differing by a named datum. **2 of 9 done** |
| B7 | bind the planes to the player, do not lower the tick. **The binding lands on Hubbard** ([prototype-lifter.md](prototype-lifter.md) §1, §3-§6): Commando song 1 binds from its certified artefacts and **0 hints** to an object with no `program` key that `universal.py` renders at **0 divergences over 11,780 ticks and 133,109 writes** — S6's roles and T2's cursors bound to the player's own cells, the score's events carrying §3.6's fields and not the bytes a fetch read, T1's three records rendered into §5's, and the residue 26 rows of 50 assignments over 18 cells, every one a cell S6 names. **1.42× the load band against the hand's 1.36×**, where the lowering it replaces was 2.46×. *Guldkornekspressen Intro* runs through the same emitter with no family branch, binds its schedule, its tuning, its 19 instrument records and 2 of T1's 5, and diverges at tick 0: its fetch **stages** the row, so the value §3.6's `note` is bound from is stored one phase later and no event states one (§7). `meta.stage` is the datum that would carry it | large | the emitter binds and does not lower: no `sets` target is an SSA temp and no cell is named by an address; 0 divergences over the whole horizon. **1 of 9 bound and certified; the second family's residue is §7** |
...
binding still cannot state are [prototype-lifter.md](prototype-lifter.md) §7,
and what the lowering it replaced measured is its §8.
