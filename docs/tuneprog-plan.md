...
| code | `deity_informant/tuneprog/`, 58 modules, 16,891 lines, none over 500 except `pipeline.py` (513); 714 hermetic + 62 HVSC + 10 oracle tests, 96 % coverage; SSA 1.0–1.6 statements per instruction; `tools/tuneprog_certify.py`, `tuneprog_recert.py` (51/51), `tuneprog_period.py`, `tuneprog_ghidra.py`, `tuneprog_floor.py`, `tuneprog_nmi.py`, `tools/survey/` |
...
| merged PRs | #225–#277, one stage each, every one on green CI with recert reproduced |
...
| item | mechanism | evidence | owner | size | acceptance |
|---|---|---|---|---|---|
| **two identity folds left** | `x ^ $FF` → `~x`; the free-standing `1 - (a cmp b)` → the negated compare | *12-run*: 19; 17 free-standing. #277 landed the other two where they belong: `1 - (a cmp b)` inside the borrow chain (`halves._notc`, the 74 borrow sites) and `carry(a + b) \| carry((a + b) + 0)` as `halves.zerofold`'s left-hand identity, which `texture.zerocarry` now runs over the view; `c = 1` is not this rewrite — `carry((a + b) & $FF, 1)` is not `carry(a + b + 1)` | idioms, texture | small | hermetic identity tests; S4 JSON byte-identical for all certificates (the folds run in the S6 pass) |
| **range-gated folds via `ranges.py`**, no e-graph | `expr_range` gates: post-increment compares `(v + 1) < 3` → `v < 2` (wraps at `$FF` without the range); the interval-decided masks and compares of `eqrules._masks`; the if-diamond borrow of `eqrules._select` as a `texture` rewrite | *12-run*: 42 S6 post-increment shapes. P-EQSAT: the diamond borrow fired at 3 sites (Follin `$6650` 6 lines → 1, both SW `init` sign extensions); `ranges.py` is 76 lines, 3–4 fixpoint rounds on all certificates. #277's cell-pair diamond does *not* reproduce them and `_select` stays: verified against `--eqsat` on `sw-emomyst`, where `if b >= 0: a = t else: a = t - 1` still folds only under the e-graph — the arms differ by one over a byte, not over a pair's own cell | idioms, texture, ranges | small | the three sites reproduce `--eqsat`'s print; a gated rule tested proved and unproved |
...
1. **expression layer** (§2.3): the two folds left, range-gated folds, then retire `--eqsat` — the folds must land first so nothing `--eqsat` measured is lost.
2. **dead values and loop/join idioms** (§2.4) — after the views, so liveness sees the final expressions; §2.2's two open rows both wait on `unroll`'s row alignment, which is the same lever.
3. **data as data** (§2.1) — needs `const` typing from the partition.
4. **complexity consolidation** (§2.7) — after the three view stages exist.
5. **P4 Ghidra oracle** (§2.6) — independent; interleave anytime.
