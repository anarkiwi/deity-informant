...
| | |
|---|---|
| certified | Automatas (defMON, both SID models), Commando songs 1–2 (Hubbard), Ghouls'n'Ghosts (Follin, 32 subtunes + `--songs all`), GoatTracker 2 ×2, SID Wizard ×2, JCH NewPlayer V20 ×2, the first two of the installed-handler family (Becher's *Jodler*, Baumrucker's *Playful Professor*) and the first two of the dead-NMI family (Andrew Rodger's *Alien_3*, Goto80's *Jazzpjazz*, both at a 30 s horizon) — 48 certificates, 750,095 certified ticks, 0 divergences, 0 envelope traps; 41 complete via periodicity, the `--songs all` program complete on 31 of its 32 subtunes |
...
| **`trap switch`: the emitted program reaches a dispatch value the trace never produced.** The campaign's largest failure class -- **189 of 7,023 tunes (2.7 % raw, 0.5 % weighted)** -- and it takes whole families with it: Virtuoso 29/30, Ben Daglish/Gremlin 25, Element114Studio 25, Fred Gray 15, Tiny/Sound Images 12, Galway 7. `emit._term` gives a `Switch` one case per value the trace saw and traps everything else, so reaching the trap means the program computes a scrutinee the trace did not record at that block. It fails early: 86 % of all divergences are in init or the first three ticks, so this is a modelling gap, not drift. Measured association only: **159 of the 189 (84 %) have a play site writing an instruction byte** against 54 % of certified programs, and 116 of 189 also folded sibling copies. Not diagnosed -- the first job is to reproduce one Virtuoso tune and find which cell the switch is over | campaign ([survey-tuneprog.md](survey-tuneprog.md) §4) | correctness | build, jumptab, ssa, emit |
...
| **the closure boundary shows up as a divergence: `trap unverified` 47 + `trap untaken` 31 = 78 tunes (1.1 %).** An arm lifted from a sibling copy, or a branch direction nothing executed, that the program then reaches while replaying its own trace. SidFactory_II/Laxity is 23 of the 47 and is the campaign's largest divergence-only family (29/30 sampled, 380 HVSC tunes) | campaign §4 | correctness | siblings, copymerge, closure |
...
7. **Fast tracer** — *gate fired (2026-08-21,
   [survey-tuneprog.md](survey-tuneprog.md) §12)*. Tracing is **78 % of the
   30 s pass's CPU and 96.8 % of the `--until-period` pass's**; the verifier
   is 11 % and 2.4 %. Verification *matches* design §2's model once amortised
   (**13,518 calls/s**, model 10–16 k); tracing does not — **329 ticks/s**
   amortised, ≈ 96 k instructions/s at design §9.3's 292 per tick, about
   **2.9× slower** than the model's 277 k, which was taken on the prototype
   VM. The whole catalogue costs ≈ 131 CPU-hours at a 30 s horizon, ≈ 529 at
   HVSC song length (design said ≈ 300) and ≈ 1,520 for a `--until-period`
   pass at a 300 s cap. A 3× tracer puts it where the design assumed it was,
   takes the song-length campaign to ≈ 180 CPU-hours and removes most of the
   333 wall-timeouts; it changes nothing else in the pipeline.
