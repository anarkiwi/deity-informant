...
| code | `deity_informant/tuneprog/`, 58 modules, 16,809 lines, none over 500; 730 hermetic + 62 HVSC + 10 oracle tests, 96 % coverage; SSA 1.0–1.6 statements per instruction; `tools/tuneprog_certify.py`, `tuneprog_recert.py` (51/51), `tuneprog_period.py`, `tuneprog_ghidra.py`, `tuneprog_floor.py`, `tuneprog_nmi.py`, `tools/survey/` |
...
| merged PRs | #225–#279, one stage each, every one on green CI with recert reproduced |
...
1. **data as data** (§2.1) — needs `const` typing from the partition.
2. **`unroll`'s row alignment** — §2.2's two open rows and §2.4's expression-domain `for` are all the same lever: a loop the printer states by its header rather than its body.
