...
| an installed handler that chains to the KERNAL IRQ epilogue never reaches its `RTI` under the tracer, so the `irq` entry is untraceable for most of the family. `Tracer._one_call` enters the handler with the 6510 interrupt frame alone, while the real dispatch runs the KERNAL prologue at `$FF48` first, which also pushes A, X and Y; `$EA81`'s `PLA;TAY;PLA;TAX;PLA` then pops the machine's own three bytes, `while SP < start` ends one instruction *before* the `RTI`, and the block that would carry it is `trap 'unreached'`. Measured while looking for Q4's evidence tune: of the 486 `play == 0` tunes in HVSC `MUSICIANS/A`-`C` (headers screened by range request), 37 are PSID; of those 23 build and **all 23** diverge at tick 0 on `unreached`, 14 crash the generated code (a `Call` emitted with no arguments -- its own bug), and none reaches an `RTI`. Every one is identical on `main`, so it is not the contract's doing. `c64.irq_stubs` already models the whole path: the fix is an entry convention -- `Entry` carries whether the vector is CINV, the tracer and `verify._enter` push A/X/Y with the frame, and `frames.contract` gains slots 1..3 (the entry Y, X, A) beside the status at 4 | Q4 | core | machine, trace, verify, frames |
