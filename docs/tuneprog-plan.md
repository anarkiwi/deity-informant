...
| code | `deity_informant/tuneprog/`, 57 modules, 16,491 lines, none over 500; 710 hermetic + 62 HVSC + 10 oracle tests, 96 % coverage; SSA 1.0–1.6 statements per instruction; `tools/tuneprog_certify.py`, `tuneprog_recert.py` (51/51), `tuneprog_period.py`, `tuneprog_ghidra.py`, `tuneprog_floor.py`, `tuneprog_nmi.py`, `tools/survey/` |
...
| merged PRs | #225–#278, one stage each, every one on green CI with recert reproduced |
...
1. **dead values and loop/join idioms** (§2.4) — after the views, so liveness sees the final expressions; §2.2's two open rows both wait on `unroll`'s row alignment, which is the same lever.
2. **data as data** (§2.1) — needs `const` typing from the partition.
3. **complexity consolidation** (§2.7) — after the three view stages exist.
