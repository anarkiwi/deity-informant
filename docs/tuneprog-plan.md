...
| code | `deity_informant/tuneprog/`, 49 modules, 14,377 lines, none over 500 (`pipeline.py` 506 excepted); 563 hermetic + 52 HVSC + 4 oracle tests, 96 % coverage; `tools/tuneprog_certify.py`, `tools/tuneprog_recert.py` (46/46 reproduce), `tools/tuneprog_period.py`, `tools/tuneprog_ghidra.py` |
...
| `fold.outline` can leave an edge to a block it deleted: S6 then dies in `graph.preds_of` with a `KeyError` on the label. 3 of the 37 (Abbing's *Belagerung 2*, Beben's *Come What May* and *Tiger's Eye*) at a 15 s horizon; all three verify and their S4 program is clean, so it is presentation only. Located: the S4, view, expand, clean, inline, tidy and `fold16` programs all have zero dangling successors and the one after `outline` has exactly one, a covered `If` arm into a block the extraction moved into a helper. `fold.sese` proves single-entry from `preds_of` taken *before* any extraction, and `_extract` mutates the procedure candidate by candidate | this stage | presentation | fold, graph |
