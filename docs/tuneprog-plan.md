...
| the PSID **speed flag** is not in the cadence. `machine._cadence` takes `pysidtracker.cadence`, which reads the timer a tune's init programs and falls back to video; the header's `speed` word (bit *n* set = subtune *n* is CIA-timed, the host's default 60 Hz) is never consulted. Found by the oracle guard: `becher-jodler` has `speed = 1` and no latch of its own, so we tick it at 19,656 (PAL video) while `sidplayfp` ticks it at 16,422 (985,248/60) -- the CSV's own raises say so, 7 ticks apart because the tune writes every 7th. Its certificate is self-consistent (the tuneprog reproduces our own trace) but its *seconds* and period are in the wrong clock, and 28 of the 37 screened carry a set speed bit. Fixing it moves `becher-jodler`'s ticks, seconds and period, so it is a row and not this stage | #261 review | model | machine |
...
| **P-EQSAT prototype** (2026-08-21) | equality saturation over the S6 expression layer: re-express `idioms` + two `texture` passes + the `sext`/flag rules as an `egglog` rule set with an interval/mask e-class analysis fed by in-IR facts (envelope asserts, mask shapes, `jumptab` index ranges); extraction by printed-token cost with deterministic tie-breaking. Acceptance: byte-equal-or-better prints on every certificate, the two recorded range-proof refusals (`if (tempo & $80)` V-after-SBC; the branch-carried borrow) firing as analysis-gated rewrites, the view-vs-certified differential clean, and a LOC comparison of rules + glue vs the passes they would retire. Presentation-only, certificates frozen; dependency `egglog` (python). If it wins, passes migrate in consolidation stages; if not, the measurement cost one stage | medium | slotted after the campaign (whose failure distribution ranks the rewrites), or earlier on the user's call |
