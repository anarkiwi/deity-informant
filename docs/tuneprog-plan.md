...
| code | `deity_informant/tuneprog/`, 45 modules, 13,320 lines, none over 500; 491 hermetic + 43 HVSC tests, 95 % coverage; `tools/tuneprog_certify.py`, `tools/tuneprog_recert.py` (44/44 reproduce), `tools/tuneprog_period.py`, `tools/tuneprog_ghidra.py` |
...
| the tick model has no cycle budget inside a call: a tune that spends a data-driven delay between register writes moves its last writes past the point a frame-boundary sample takes them (Puterman's V20 wrapper, 168 -> 10,248 cycles from a tick's first SID write to its last; 494 of 3,000 oracle frames differ, in exactly the five registers written last). The certificate is unaffected; the trace-vs-oracle agreement is not | P-JCH (#251) | oracle, model | trace, machine |
