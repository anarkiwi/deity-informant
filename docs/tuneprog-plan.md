...
| | |
|---|---|
| certified | 51 certificates, 759,353 ticks, 0 divergences, 0 envelope traps; 42 complete via periodicity, `--songs all` complete on 31/32 subtunes. Families: defMON (*Automatas*, both SID models), Hubbard (Commando 1–2), Follin (Ghouls 32 subtunes + union), GoatTracker 2 ×2, SID Wizard ×2, JCH V20 ×3 (incl. the two-entry *Easy Does It*), installed-handler ×2 (*Jodler*, *Playful Professor*), dead-NMI ×2 (*Alien_3*, *Jazzpjazz*), patched-dispatch ×2 (*Experiment Zeta* complete at period 5,184, *Deflektor* 30 s) |
...
| survey ([survey-tuneprog.md](survey-tuneprog.md)) | 7,023-tune stratified sample at 30 s: **91.2 % of HVSC by weight certifies** (76.7 % raw), 2.5 % diverges, 6.2 % refused with a diagnosis, 0.26 % crashes; `--until-period` over 1,338: 99.4 % of certified programs complete by weight. 58 CPU-h on the old tracer; the tracer is now 3.0–3.5× faster (#271) |
| code | `deity_informant/tuneprog/`, 56 modules, 16,459 lines, none over 500 except `pipeline.py` (510); 649 hermetic + 62 HVSC + 10 oracle tests, 95 % coverage; `tools/tuneprog_certify.py`, `tuneprog_recert.py` (51/51), `tuneprog_period.py`, `tuneprog_ghidra.py`, `tuneprog_floor.py`, `tuneprog_nmi.py`, `tools/survey/` |
| baseline | Ghidra high P-code export with SMC context ([ghidra-highpcode-export.md](ghidra-highpcode-export.md)); three Ghidra oracles; `sidplayfp` grid oracle |
| merged PRs | #225–#272, one stage each, every one on green CI with recert reproduced |

## 2. Backlog

Open rows only, grouped by lever. `owner` = the modules that change. Size:
...
| **retire `--eqsat`** | delete `eqsat.py`, `eqrules.py`, `tests/tuneprog/test_eqsat.py`, the flag, `texture.clean`'s branch, the `egglog` extra; keep `ranges.py` | P-EQSAT verdict: 600 lines, ×3.6 CPU, −1.5 % tokens, 6/42 identical. Review (2026-08-22): `pick` registered once per *occurrence* of a repeated expression (`roots.setdefault(e, g.root(e))`); `_forwarded` duplicates `texture.propagate`'s candidate set; the interval domain is defined twice and disagrees (`ranges.expr_range` bounds `\|`/`^` by the next power of two, `eqrules._analysis` by the mask); extraction reads egglog's private `_serialize()` with `egglog` unpinned | eqsat, eqrules, texture, pyproject | small; after the two rows above | −534 lines, one dependency fewer, prints byte-identical with the flag off |
...
| **flag naming** | `cN` temporaries named `carry`/`borrow` by the op that defines them | *12-run*: 39 defs / 88 uses in 5 of 12 tunes; N, Z, I, D never reach the print, V once | pseudocode | small | naming only |
| the closed program (`--closure static`) is a different shape for the *covered* code — **half done (Q1a)**: closed arms own no dominance, `Je_suis_Linus` 23 → 16 `goto`, `Emomyst` 2 → 1. What is left is not dominance: a closed path rejoining inside a covered block splits it and the extra predecessor stops `ssa.merge_chains`; the remaining 16 are 7 join regions with more than one way out, 2 that both leave and return, 4 that would promote but not pay | make the covered subgraph's block boundaries independent of closed rejoins | criterion GT2 0 `goto` not met; `--closure static` stays off | structure, closure, ssa | medium | GT2 0 `goto` under `--closure static`; trace-closed prints untouched |
...
### 2.5 Fold reach and precision

| item | mechanism | evidence | owner | size | acceptance |
|---|---|---|---|---|---|
| folding a copy nothing ran costs more than it saves | a silent copy adds the columns and the `switch (v)` with no second body to remove | Follin 17-19, 25, 27, 29 grow ~6 % statements, ~20 % blocks, 24–49 printed lines each; buys per-voice names and the coverage vector; `--no-merge` is the escape | copymerge | — | accepted boundary unless a rule makes the silent copy free |
| an edge into another copy at a row that is not that copy's entry (*Automatas* `$16AB`) | lowering as `v += 1; goto` the template row is sound and folds it, but the merged body gets two entries, costing a `goto` and the `ad`/`ctrl` names | measured, refused as the narrower rule says | copyrows | — | accepted boundary |
| a copy's stream runs past a jump into unreached code (the Knob's `$1167`), carried as an unverified row | ending the stream there needs `jumptab`'s transfer facts, not the image alone | measured and rejected on the image: it shatters Ghouls' 3×237 families to 3×2 and breaks closure invariance on song 31 | siblings, jumptab | small | the Knob row gone, Ghouls families unchanged |
| the phase of a repeating cascade is a convention where two readings tie exactly (*Automatas* `p_168C` `$172C` vs `$1734`) | pin by `slack` then lowest base | refusing ties also refuses the two real 5-copy cascades | siblings | — | recorded convention |
| a fold takes cells the naming plane had (the Knob's `$17B9`/`$17BC` → `b17B9[v + 3]`, Commando 2's `$54F8`) | the merged access's region should keep the field names of the cells it unites | measured in P1 | copymerge, views | small | the named fields survive the fold |

### 2.6 Certification and coverage
...
| the 30 divergences and 8 `JAM` crashes left in the NMI class at 30 s | `state hash` 11, `sid` 6, `io` 5, `entry register` 3, `input mismatch` 2, one each `switch`/`input exhausted`/`brk`; 18 of 30 past tick 5 — the schedule drifting, not a first-tick gap | Q8 | trace, verify | medium | per-class diagnosis |
| `trap switch` over an unmatched return (the RTS trick) | the program pops a return address the trace did not; all four are `stack: residual` — the stack model's boundary | 4 of the 189: *Exterminator*, *Ocean_Loader_3* (init), *Blood_n_Guts* (tick 0, pops `$0001`), *Gyruss* (tick 356) | stack, cfg, frames | medium | the four certify or refuse with the frame diagnosis |
...
| the `io` write list differs, 73 tunes (1.0 % raw, 0.2 % weighted), 41 in init | VIC/CIA writes are not the trace's; SID never differs | SIDSys18.6 17, DigitalArts 11, Novaload 11 | build, verify | medium | per-family diagnosis |
| volatile-input replay: `trap input exhausted` 26 + `input mismatch` 12 | pinned inputs consumed in a different order or number | 38 tunes (0.5 %); Novaload and DigitalArts lead | verify, interp | medium | diagnosis |
...
| a write to `$D000`–`$DFFF` with I/O mapped also writes the RAM under it (`tracevm.write`, `interp.iostore`) | two planes, chip and RAM beneath | unobservable in every exemplar; 3 tunes in the survey discriminate | tracevm, interp | — | waits for a discriminating tune |
...
| the tracer counts CPU cycles where the sampler's clock also spends VIC DMA (57–60 cycles per frame, +533 inside one Knob tick) | a raster model, only if a comparison ever needs sub-frame time | free today: both sides framed by the interrupt | tracevm, machine | — | not needed |
...
| the sweep's `cpu_trace` bills S0 entry discovery to S1; a `no entry` refusal costs 14.6 CPU-s each (46 of 60 refused tunes) | a `cpu_entry` column; find out why `no entry` runs a whole init trace on `pysidtracker` | a certified tune's trace CPU is 8.6 % `_traced`; a refused tune's ~100 % and did not move with the 3× tracer (775 → 765 s over 60) | survey/tuneprog_sweep, machine | small | the split reported; `no entry` cheap |
| **P4 Ghidra oracle** | per-call input capture in the facts export; resolve the two emulator disagreements (GoatTracker, Follin from call 2); Ghidra function bodies vs clone-per-entry (`ghidra_partial`); the complexity/coverage/emulate oracles in the nightly recert (`ours_bigger` flag) | Ghidra's emulator agreed byte-for-byte on Automatas and Commando | ghidra_facts, ghidra_compare, headless | medium | independent; interleave anytime |
| opcode cells whose alternative is not `RTS` in the SLEIGH export | overlay or paired constructor | 198 of 7,023 (3.4 % weighted) have one; the `RTS`-only overlay covers the minority | ghidra/6510 | medium | baseline only |

### 2.8 Complexity and duplication
...
| private helpers sharing a name across modules | bounded census: `_accesses` ×3 (copyview, regions, views), `_split` ×3, `_match` ×3, `_kind` ×3, `_node` ×4 — each set either one mechanism (merge) or renamed to say how they differ | 2026-08-22 census of `deity_informant/tuneprog/` | — | small | no two private helpers share a name unless they are one |
| `texture.clean`'s eqsat branch | goes with the retire row (§2.3) | — | texture | with §2.3 | — |

## 3. Done

One line per struck row: title · PR/date · headline · record.

- bounded static closure of untaken directions · P2 · `trap 'untaken'` 18/15/28/49 → 5/0/3/1 on four exemplars, off by default (`--closure static`) · design §3
...
- *Automatas*' row-advance blocks · P1 (#248) · one 45-row body over 22 columns, by the copy index
- an edge leaving one copy for another refuses the family · P1 · the rule held, ownership was the refusal; Follin's 8 one-voice effects and `$112A` fold
- a merged family's patched dispatch loses the static table closure · P1 · per-copy enumeration, 3 → 39 arms on Follin song 1
- a merged loop prints as `while` over `copies_XXXX[...]` · #244 · `copyview.py`, `loops.copies` → `for v in 0..k-1`
- `unroll` substitutes copy 0's constants · P1 · a constant that does not step keeps its literal
- k prologues make no `for` · Q1a · the prologues are the step (`loops._chain` proves the chain by assignment)
- a merged access unites its regions, roles lost · Q1b · two measured causes (`facts.sid_image` on constant addresses; block-local reach), +4 `cursor`, +30 `table` roles, none lost
- sibling discovery from block boundaries · Q2 · candidate bases and the chain relation from the image; property tests over six seeds × four shapes
- periodicity proof by lcm · P2, refuted · `period.py` classifies `periodic`/`state only`/`aperiodic`; Commando's pulse-width accumulators are the observable (41,898 of 48,192 tick write lists differ at the loop)
- second interrupt schedule (NMI + IRQ) · Q8, 2026-08-22, #272 · the CIA #2 NMI is the schedule's second entry; `cia.py`, `nmi.py`; 130 of the 195 class certify at 30 s · [prototype-nmi.md](prototype-nmi.md)
- the NMI refusal sized on evidence · Q5, #269 · a CIA #2 source can fire iff the accumulated ICR enables it and the timer runs; 547 refusals → 311 armed / 81 dead / 154 undecided
- a CIA #2 latch as the play cadence · Q5 · `_cadence` takes a CIA period only when it is CIA #1's
- sign-extension and flag-algebra printing · Q1b · `A + sext(T)`, `overflow(A - B)`
- index range for jump-table extents · P1 · intraprocedural (Follin 23 vs 21 arms)
- `--songs all` resume state · P2 · each subtune carries `{calls, stop, horizon}`
- printer memo invalidation with 16-bit views; `node_exprs` guard · P2
- a subtune stopping on a period certified at the chunk boundary · Q3 · `pipeline._certified` reads the witness (`ghouls-songs-all` 220,049 → 111,763)
- the transpose stride view · Q1b · `views.transpose_split` (JCH's three tracks as records)
- SID register offset from a per-track table · Q1b · `facts.voice_maps`; `sid[v].ad` for `sid.reg[5 + voice[v].b1740]`
- a loop body's joins are `goto` · Q1a · a region with one way out promotes; 7 → 0 `goto` on both V20 tunes
- comparing a tick's writes against a sampler · Q3 · `grid.py` frames both sides by the interrupt; Knob 0 of 3,000 frames
- numba tracer · refuted 2026-08-22 · the compiled P-Code was 5–6 % of self time; see fast tracer
- stack elimination in S4 · #237 · `SP` absent from all exemplar `tuneprog.py`; the page outside the footprint both sides
- an `RTI` entry tune is residual · Q4 · the pushed frame is the entry's contract (`frames.contract`)
- `--until-period` stops before the page-inclusive repeat · Q4 · the trace stops on the witness the stack status allows
- an installed handler chaining to the KERNAL epilogue never reaches `RTI` · #261 · the entry's vector decides its frame; 37/37 trace and verify, 34/37 certify · [prototype-kernal-entry.md](prototype-kernal-entry.md)
- 14 of the family crash on a `Call` with no arguments · #261 · the IR call graph has cycles; `wire.wire` is a fixpoint
- the `kernal` decision was written-vector precedence · #262 · the 6510 port's HIRAM decides; `machine.vector_gate` refuses `vector banked out`
- the CINV frame convention had no oracle guard · #262 · `test_oracle.py`: *A Mind Is Born* 0 of 3,000 frames differ
- the PSID speed flag is not in the cadence · #264 · host CIA at `$4025`/`$4295`, per subtune
- `trap switch`, 189 tunes · Q6, #270 · three front-end mechanisms (patched `JMP (ind)` operand, zero branch offset, copy-index step); 177 of 189 certify
- the closure boundary as a divergence, 78 tunes · closed by Q6 · 78 → 78 certified; the diagnosis was wrong
- P1 fold reach · #248
- P2 certificate accounting · #250
- P-JCH V20 family · #251 · both tunes complete, 0 divergences, five generic machine-model fixes
- Q1a structuring · 2026-08-21
- Q1b views & naming · 2026-08-21
- Q2 sibling discovery deep fix · 2026-08-21
- Q3 sweep prerequisites · 2026-08-21 · `grid.py`, witness-based horizons, one tune map
- Q4 = P3 stack residual policy · 2026-08-21
- P-EQSAT prototype · 2026-08-21, #265 · the prints qualify (no metric worse, −1.5 % tokens), the cost does not (600 lines, ×3.6 CPU); keep `ranges.py` · §2.3 rows
- P-FLOOR Commando · 2026-08-21, #266 · the gap is storage typing, not algebra: 252 → 115 lines under four typings, `--eqsat` moves 8 tokens · [prototype-commando-floor.md](prototype-commando-floor.md)
- Q5 NMI refusal · 2026-08-22, #269
- Q6 `trap switch` · 2026-08-22, #270
- Q7 fast tracer · 2026-08-22, #271 · the site is the VM's cache key; 3.0–3.5× (480–580 k instr/s), `Trace` byte-identical over 82 traces, recert 50/50
- Q8 NMI prototype · 2026-08-22, #272 · 181 of 7,023 have a dispatching NMI beside a play entry (1.3 % weighted); `jch-easy-does-it` 1,799 ticks, 199,514 preemptions, 0 divergences
- campaign at survey scale · 2026-08-21, #267 · [survey-tuneprog.md](survey-tuneprog.md)
- gates: the copy index as an IR value (#241, #242, #244) and the stack-free executable (#237) — both met; Follin song 1 1,229 statements / 441 blocks → 671 / 254 as one `for v in 0, 1, 2`
- the remaining exemplars: Blackbird, Galway, Walker certify at 15 s unchanged
- design claims held (from v3 §2): per-tick equivalence checkable cheaply; the dynamic front end needed no tune-specific code for any anatomy mechanism; envelope traps never fired; SSA removes flags and registers (1.0–1.6 statements per instruction); periodicity upgrades a horizon; Ghidra is a baseline, not a core (high P-code 5.8–10.6× our S4)
...
- Ghidra's emulator disagreements from call 2 (GoatTracker, Follin) are per-call inputs the facts export does not carry — bounded, not a design hole (§2.7 P4).
...
6. **complexity consolidation** (§2.8) — after the three view stages exist.
7. **P4 Ghidra oracle** (§2.7) — independent; interleave anytime.
