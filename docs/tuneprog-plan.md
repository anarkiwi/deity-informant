...
| code | `deity_informant/tuneprog/`, 45 modules, 13,196 lines, none over 500; 478 hermetic + 35 HVSC tests, 95 % coverage; `tools/tuneprog_certify.py`, `tools/tuneprog_recert.py` (42/42 reproduce), `tools/tuneprog_period.py`, `tools/tuneprog_ghidra.py` |
...
measure stale; P3 follows the campaign's class sizes. P1 and P2 are done; the
campaign is next, and it is what re-ranks the two rows P2 left open (the
periodicity reduction and the closed program's structuring).
