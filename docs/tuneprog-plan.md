...
| code | `deity_informant/tuneprog/`, 54 modules, 15,757 lines, none over 500 (`pipeline.py` 510 excepted); 621 hermetic + 60 HVSC + 10 oracle tests, 95 % coverage; `tools/tuneprog_certify.py`, `tools/tuneprog_recert.py` (50/50 reproduce), `tools/tuneprog_period.py`, `tools/tuneprog_ghidra.py` |
| at survey scale | the whole pipeline over the stratified 7,023-tune sample ([survey-tuneprog.md](survey-tuneprog.md), §8 item 1): **91.2 % of HVSC by weight certifies at a 30 s horizon** (76.7 % raw), 2.5 % diverges, 6.2 % is refused with a diagnosis, 0.26 % crashes; the `--until-period` pass over 1,338 of them leaves **99.4 % of certified programs complete by weight**. 58 CPU-hours at the tracer that campaign ran on; **the tracer is now 3.1-3.6× faster** (§8 item 7, #271) and a certified tune's trace CPU falls 2.89× on a 200-tune re-run of the same sample — the part of that 58 that does not move is entry discovery, which the sweep bills to the trace stage |
...
7. ~~**Fast tracer**~~ — *gate fired 2026-08-21, **done 2026-08-22** (#271)*.
   ...
   **One mechanism**: the site key *is* the VM's cache key, so everything
   ...
   |---|---|---|---|---|---|
   | *Automatas* (defMON, CIA) | 12,029 | 798 ticks/s | **2,556** | **3.20×** | 513 k |
   | Commando song 1 | 1,503 | 626 | **2,228** | **3.56×** | 589 k |
   | Ghouls song 1 (Follin) | 1,503 | 1,222 | **3,751** | **3.07×** | 483 k |
   | GoatTracker 2 *Do It Again* | 1,503 | 480 | **1,587** | **3.31×** | 520 k |
   | JCH V20 *Guldkornekspressen* | 1,503 | 439 | **1,463** | **3.34×** | 520 k |
   | *Experiment Zeta* `--until-period` | 6,000 | 586 | **2,000** | **3.41×** | 560 k |
   | *Automatas*, 40,000 ticks (footprint stress) | 40,000 | 757 | **2,403** | **3.18×** | 511 k |

   **3.1-3.6×, ≈ 480-590 k instructions/s**, so the production tracer is now
