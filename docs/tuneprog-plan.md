...
| | |
|---|---|
| certified | Automatas (defMON, both SID models), Commando songs 1–2 (Hubbard), Ghouls'n'Ghosts (Follin, 32 subtunes + `--songs all`), GoatTracker 2 ×2, SID Wizard ×2, JCH NewPlayer V20 ×3 (the third is the sample build *Easy Does It*, the two-entry certificate), the first two of the installed-handler family (Becher's *Jodler*, Baumrucker's *Playful Professor*), the first two of the dead-NMI family (Andrew Rodger's *Alien_3*, Goto80's *Jazzpjazz*, both at a 30 s horizon) and the first two of the patched-dispatch families (NecroPolo's *Experiment Zeta* on the Virtuoso engine, complete on a 5,184-tick period; Ben Daglish's *Deflektor* at 30 s) — 51 certificates, 759,353 certified ticks, 0 divergences, 0 envelope traps; 42 complete via periodicity, the `--songs all` program complete on 31 of its 32 subtunes |
| certify at 15 s, not yet run to length | Blackbird (Quintessence), Galway (Comic Bakery), Walker (Chameleon) |
| ~~refused by design~~ | *was* JCH Easy Does It (NMI sample mixer = second interrupt); **certified 2026-08-22** ([prototype-nmi.md](prototype-nmi.md)), and 130 of the 195 tunes of its class with it. What refuses now is only a CIA #2 source with no schedule (TOD alarm, serial, FLAG, a CNT timer: 6 tunes of 7,023) |
...
| ~~second interrupt schedule (NMI + IRQ sharing regions)~~ *done (Q8, 2026-08-22)*: a CIA #2 NMI is the schedule's second entry and a second procedure ([prototype-nmi.md](prototype-nmi.md)). `cia.py` is the chip (Timer B on cycles or on Timer A, one-shot, the latched flags and the edge-triggered line, a latch write that lands at the *pending* underflow rather than reloading); `nmi.py` says when the line asserts and which vector carries it; the tracer takes the NMI at the instruction boundary it is due at -- inside a tick and in the host's idle time -- and records the preemption schedule as data; S8 replays it at **store** granularity, replaying the second entry's live-in registers from the schedule. Two properties the replay rests on are **checked per NMI**, not assumed: store separability (a play load inside an open preemption window that reads a cell the handler stored in it refuses `schedule not store-separable`) and register preservation (`nmi clobbers registers`); `compared` carries `"nmi store separability"`. Measured: **195 tunes of 7,023 have a dispatching NMI beside a play entry, 181 of them with a classified schedule** (1.3 % weighted), and the two entries **never share code** (0 of 181), which is what lets the pinned input stream partition by entry. 130 of the 195 certify at 30 s (30 diverged, 27 refused, 8 `JAM`). Zero cost on a one-entry program: its emitted text is byte-identical, it runs on the plain `Machine`, recert 51/51 | design §10, JCH | scope | machine/trace/verify |
| the NMI instant is early without a VIC-DMA model, so a CPU a badline stalled takes the NMI later than we do. **Not the only cause, measured**: the `$0318` path spends 7 cycles in `$FE43`'s `SEI; JMP ($0318)` that the tracer skipped, and modelling it (`nmi.KERNAL_STUB`, keyed on the entry's `kernal` flag) moves Mixer *Iisibiisi*'s instants from 1,781 to 4,119 within 2 cycles of an oracle instant and from 5,503 to 12,356 within 8, of 64,664 (8.5 % → 19.1 %), median −1 → +8; *Easy Does It* takes the raw `$FFFA` and is unmoved (256 / 936 of 21,149). So on the `$0318` path — **84 of 181 tunes, 85.3 % of the class by weight** — half of the offset was the stub, and the VIC is what is left. Everything the play routine writes is 0 frames differing over 1,500 frames on three tunes of three classes; `$D418`, which only the mixer writes, differs in 10 / 21 / 54 % of frames by one sample step, and 344 of 1,500 frames place a `$D418` write between two different play writes | Q8 | oracle | trace, a raster model |
| `frames.contract` names the frame only for the *tick* entry, so an NMI procedure's pushed status byte is an unresolved slot and the whole program stays `stack: residual` (`jch-easy-does-it`: depth 8, held by `nmi`). Extending it wants the recorded status pinned into the flag registers too, since an NMI's status is the *interrupted* program's, not the handler's own | Q8 | presentation, certificate | frames, verify |
| the second entry's live-in state is **replayed, not computed**: the SP, pushed status, return pc and A/X/Y come from the schedule row (`nmi.REPLAYED`, counted as the NMI schedule entry's `replayed_registers` — 6 values × 199,514 NMIs = 1,197,084 on `jch-easy-does-it`, beside its 206,710 `inputs_pinned`, which are 199,514 `$DD0D` acknowledge reads plus 7,196 tick-side). One consequence is visible in the decompiled mixer: the `LDA #`/`LDY #` operand bytes JCH saves A and Y into are reproduced from the schedule. Passing them instead as the `nmi` entry's arguments from the preempted procedure's live values — the way `frames.contract` passes the status byte — would compute them, and would settle the `stack: residual` row above | Q8 | certificate, honesty | verify, frames, build |
| `schedule not store-separable`: a load of the interrupted routine inside an open preemption window that reads a cell the handler stored in that window has no place in the schedule, so it is refused. **6 tunes of the 195 class at 30 s, 5 of the 7,023 sample, 7 more in the released banked-out class.** Only that direction needs checking (the interrupted routine makes no store inside its own window, and `$D000`-`$DFFF` reads are pinned per entry). What would certify them is a finer schedule — a preemption point per *load* of a stamped cell, at the replay cost that implies | Q8 | correctness | nmi, interp, verify |
| the stack page is a second direction the store-granularity replay does not make exact: `NmiMachine.at` runs the pending NMI at *any* store, counted or not, while the schedule counts only stores outside the stack page, so an IR store wholly inside `$0100`-`$01FF` (a residual stack, a tune using the page as a table) between the last counted store and the NMI's instant replays the handler *before* that store where the trace had it after, and the separability check does not see it (the epoch does not move on a stack byte). The converse rule -- no hook on a stack-page-only store -- was **measured and rejected (2026-08-22)**: `jch-easy-does-it` diverges at tick 6 on the state hash, because the NMI's pushed frame and the play routine's own pushes share the page and their order is the residue the page-inclusive hash sees. Exact placement needs the instruction index the trace already records, which the emitted program cannot count, or a stack store count both sides share, which stack elimination removes. Caught only where the reordering reaches a SID write or the hash; narrow (an explicit stack-page IR store) and undiagnosed in population | Q8 review | correctness | nmi, interp, verify |
| `nmi clobbers registers`: a handler whose `RTI` does not return the A/X/Y it interrupted cannot be replayed from the schedule row, so an in-tick NMI that does it is refused (an idle-time one is exempt; the next tick's entry registers are verified anyway). **12 tunes of the 195 class at 30 s, 8 of the 7,023 sample, 4 more in the released banked-out class.** Passing the interrupted registers as the entry's arguments (row above) is what would build them | Q8 | correctness | nmi, verify |
| `tools/tuneprog_recert.py --resume` replays the previous tree's verdicts when one `--out` is reused across a code change: `recert.json` records what was reproduced but not *which* tree reproduced it, so a stale state file is read as this tree's. A fresh `--out` per tree avoids it; stamping tree identity (HEAD, or the certificate-relevant module digests) in `recert.json` fixes it | Q8 | tool | tuneprog_recert |
...
| ~~**Q8 NMI prototype**~~ *done (2026-08-22)*: §8 item 3's last design gap. A CIA #2 NMI is the schedule's **second entry** — one `Trace`, two procedures (`tick` and `nmi`), the preemption schedule recorded per tick and replayed by S8 ([prototype-nmi.md](prototype-nmi.md)). The population is the first deliverable and it re-ranked the rest: `tools/tuneprog_nmi.py` classifies all 7,023 by what the handler does, and **195 have a dispatching NMI beside a play entry, 181 of them with a classified schedule (2.6 % raw, 1.3 % weighted)** in four classes (`sample player, silent play` 67, `sample mixer ($D418 only)` 57 and 1.0 % weighted, `no SID write` 49, `handler writes the register file` 8), against §5b's 311/1.8 % — that count was `nmi_gate`'s verdict over the 547 tunes the campaign had already refused, so it included tunes an NMI model cannot help. **The two entries never share code (0 of 181)** and **do share RAM both ways (67 % weighted each)**, which decided the model: two procedures, and a schedule placed at *store* granularity, with the one direction that makes exact checked per NMI rather than assumed (`schedule not store-separable`). Three chip corrections were needed and the third mattered most — Timer B (29 % of the class), one-shot mode, and a latch write to a *running* timer landing at the pending underflow instead of reloading the counter (JCH's mixer rewrites `$DD04` every NMI; reloading made it 26 % slow and broke the grid from frame 584). Against `sidplayfp` over 1,500 frames on three tunes of three classes, **`$D400`-`$D417` is 0 frames differing on all three and the changed-write list is identical in order in 1,500 of 1,500**; `$D418` differs in 10/21/54 % of frames, and 344 of 1,500 place a `$D418` write between two different play writes, because the NMI instant is early — half of it the `$0318` stub now modelled, the rest the VIC (§5 row). **`jch-easy-does-it`** is the certificate: 1,799 ticks, 199,514 preemptions, 1,197,084 replayed registers, 0 divergences, 0 envelope traps, the schedule in the document. Two properties of the replay are **checked per NMI** rather than assumed — store separability and register preservation — and the oracle gate on what the play routine writes did not fire. Class-wide at 30 s: **130 of 195 certified** (all 195 used to refuse), 30 diverged, 27 refused, 8 `JAM`; separability cost 4 certificates and register preservation none. The pre-init port refusal went with it: `find_entries` no longer refuses over a port init moves, which released 317 tunes from `vector banked out` — 175 of them certify, all 201 built programs the same `entry: irq, kernal: false` shape. Cost on a one-entry program is **zero** — the emitted text is byte-identical (sha1 unmoved on three tunes), it runs on the plain `Machine`, verify throughput is unmoved and the tracer's is within 1-3 % — and recert is **51/51 with 0 fields moved** | §8 item 3, design §10 | medium-large | machine, cia, nmi, trace, cfg, build, emit, interp, verify, grid, tests, docs |
...
3. ~~**The remaining exemplars**~~ / ~~**a JCH second-interrupt prototype**~~ —
   *the second half is **done (2026-08-22)**, §5b's Q8 row and
   ...
   `jch-easy-does-it` is the certificate; 130 of the class's 195 certify at 30 s.
   **The gate did not fire**: against `sidplayfp` over 1,500 frames on three
   tunes, `$D400`-`$D417` — everything the play routine writes — is 0 frames
   differing on all three and the changed-write list is identical in order in
   1,500 of 1,500; the disagreement is `$D418`'s placement (10 / 21 / 54 % of
   frames, 344 of 1,500 with a `$D418` write between two different play writes),
   which is the NMI instant and not the write list.
