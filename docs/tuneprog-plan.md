...
| code | `deity_informant/tuneprog/`, 49 modules, 14,451 lines, none over 500 (`pipeline.py` 506 excepted); 566 hermetic + 52 HVSC + 5 oracle tests, 96 % coverage; `tools/tuneprog_certify.py`, `tools/tuneprog_recert.py` (46/46 reproduce), `tools/tuneprog_period.py`, `tools/tuneprog_ghidra.py` |
...
| **data-gated: now measured** ([survey-tuneprog.md](survey-tuneprog.md) §10, 7,023 tunes) | NMI+IRQ = `second interrupt source armed`, **547 tunes, 45 % of all refusals, 3.0 % of HVSC by weight** -- the largest addressable population in the campaign, §8 item 3's prototype is now first by data · two-planes chip/RAM **3 tunes** (34 read the RAM under I/O at all): the discriminating tune exists and the class is negligible, so the row stays closed · residual-stack localisation **826 tunes / 4.4 % weighted**, and the read is inside `tick` 62 % of the time · periodicity: **5,051 of 5,384 certified programs (95.7 % weighted) find no state repeat in 30 s**, so the reduction is gated on the `--until-period` pass, not on a sound shape · naming, numba (§8 items 4, 7) | — | |
