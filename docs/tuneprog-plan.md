...
| ~~index range for jump-table extents (Follin 23 vs 21 arms)~~ *done (P1) intraprocedurally: the branches on the one path into a dispatch prove a range for the index (sign test, equality, compare), and a merged copy's table is bounded by the sibling bases one index reads -- the second is the extent rule reading what the merge knows rather than a proof, and like the extent rule it only ever claims less -- Follin's three dispatches are 21 arms each, none displaced. Not built interprocedurally: SID Wizard's two dispatches do take their index as an argument, but what the caller's `CMP #$60` proves ([96, 256)) is already inside the extent (123..127 and 125..127), so the walk would cost code and buy nothing measurable* | SW | closure | jumptab |
