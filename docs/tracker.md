...
`LOOKUP` value, a dropped ordered write, and two swapped ordered writes are each
detected.
...
Generator = (transfer, trigger, route)
  transfer : DIV(n)                  # one tick per n input triggers (a clock)
           ...
           | RAW(per_frame)          # replay writes verbatim: the floor
...
`ctrl`, `pw`, `ad` and `sr` are **not** interpreted yet: they render from `RAW`.
The coverage numbers in §5 say so plainly; nothing is claimed that a generator
does not reproduce.
...
## 5. Coverage (measured, 200 frames unless stated)
...
| tune | pitch table | interpreted | freq plane |
|---|---|---|---|
| Commando (Hubbard), 300 frames | `$5428` interleaved, 97 words | **1198/2525 = 47.4%** | 1198/1280 = 93.6% |
| Ghouls_n_Ghosts (Follin) | `$6D35`/`$6D96` split, 97 | 444/1164 = 38.1% | 444/722 = 61.5% |
| Automatas (Goto80/DefMON) | `$1578`/`$1614` split, 120 | 398/4800 = 8.3% | 398/1200 = 33.2% |
| Athena (Galway) | `$C517`/`$C55F` split, 72 | 426/1882 = 22.6% | 426/1200 = 35.5% |
| Krakout (Daglish) | `$E629` big-endian, 12, octave-shift | 228/5000 = 4.6% | 228/1200 = 19.0% |

`ctrl`/`pw`/`ad`/`sr` are 0% interpreted on every tune above.

Over the first 60 cached HVSC tunes (57 decompile; keyed by full relpath): a
pitch table is recovered for **53**, the law passes for **57/57**, and the freq
plane is **70.1%** interpreted (37890/54067) against **53.3%** (28796/54067) for
the pre-frameprog lift over the same window.

## 6. Where the residual goes next
...
1. **ctrl/gate + ADSR** — the control automaton as `LOOKUP` nodes triggered on
   the note-on edge (the biggest residual plane on Commando).
2. **pw** — the pulse-width accumulator as a `RAMP` with bounds.
...
4. **Arrangement** — orderlist/pattern/transpose as `LOOKUP` nodes routed to
   `Fire`, with shared subgraphs for reuse and a back-edge for the loop.
