...
|---|---|---|---|---|---|
| freq | 395264 | 602528 | 65.6% | 395264 | 0 |
| pw | 20452 | 561585 | 3.6% | 20442 | 10 |
| sr | 17593 | 115963 | 15.2% | 13111 | 4482 |
| ad | 16522 | 112534 | 14.7% | 12479 | 4043 |
| ctrl | 9604 | 300573 | 3.2% | 6230 | 3374 |
| filter | 0 | 240694 | 0% | 0 | 0 |
| **all** | **459435** | **1933877** | **23.76%** | **447526** | **11909** |

**97.4% of interpreted emits rest on strong evidence** — a declared byte at a
recovered row, or generated from one. The shallow 11909 are program immediates
(`ctrl`/`ad`/`sr` releases and hard-restarts) plus the 10 observed seeds the pw
sweeps start from; they pass the law without explaining an index and are never
folded into a strong figure. The freq plane splits 292295 pitch-table `LOOKUP`
emits (the note lane, §4) and 102969 declared-lane `SELECT` emits (§4b); `pw`'s
20442 splits 20161 declared-lane reads and **281 generated by a sweep `RAMP`**
(§4c). `filter` is untouched at this layer — every filter write is still `RAW`.

The three planes that dominate the residual are `pw` (541133 emits), `ctrl`
(290969) and `filter` (240694). `pw` and `filter` are the same problem and §7.1 is
the one change for both: their parameters are staged in RAM. `ctrl` is a different
problem and not that change's to fix — it needs a row for the gate to ride, and a
voice that never reads its waveform from a declaration has none (§8).
...
1. **Parameters staged in RAM** — the sweep is a `RAMP` wherever its step is
   ...
   table into RAM at note-on. This is a frameprog change, not a tracker one.
   `frameval` already maintains the cell → origin map *through* non-SID stores
   (docs/frameprog.md §1.4) — a RAM cell copied from a declared table already has
   its origin recorded. What is missing is **reporting**: origins are surfaced
   only alongside SID store sources, so the tracker cannot ask "what does this
   accumulator's step cell trace back to". Exposing the map for an arbitrary cell
   makes RAM-staged steps, bounds and rates declared bytes at recovered rows, and
   it unlocks the pw residual, the filter plane and every other parameter the play
   code stages.

   **Measure before building.** The population is bounded and known: 329792 pw
   emits whose byte comes from a cell outside every declaration (§6), plus the
   filter plane's 0%. The step that shipped §4c returned 291 emits against a
   fitting ceiling of 7480, so the ceiling is the thing to establish first — how
   many of those 329792 resolve to a declared origin under the extension, reported
   as a number, *before* any generator work. A refinement whose ceiling is not
   measured is a refinement whose value is not known.
