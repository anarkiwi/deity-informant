...
// the level is how many enclosing regions the exit leaves, innermost 1 (absent);
// a `loop` whose body always breaks is a scope, so this is the only form a
// forward merge needs (DREAM, NDSS 2015; Peterson/Kasami/Tokura CACM 1973)
flowline: "goto" HEX _NL -> fl_goto
        | "unobserved" HEX _NL -> fl_unobs
        | "continue" [INT] _NL -> fl_cont
        | "break" [INT] _NL -> fl_brk
